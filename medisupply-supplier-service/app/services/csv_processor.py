...
from app.schemas.product_schema import ProductoCSV, ErrorValidacion, ResultadoCargaMasiva, ProductoResponse
...
class CSVProcessor:
    ...
    def procesar_archivo(self, file_content: bytes, rechazar_lote_ante_errores: bool = True) -> ResultadoCargaMasiva:
        """
    ...
    def _validar_columnas(self, columnas_archivo: Optional[List[str]]) -> List[ErrorValidacion]:
    ...
    def _procesar_lineas(self, csv_reader, rechazar_lote_ante_errores: bool) -> ResultadoCargaMasiva:
        ...
        for linea_num, fila in enumerate(csv_reader, start=2):  # Empezar en 2 (después del header)
            total_lineas += 1
    ...
    def _procesar_linea_individual(self, fila: Dict[str, str], linea_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for error in e.errors():
                errores.append(ErrorValidacion(
                    ...
                    error=error['msg'],
                    valor=str(fila.get(error['loc'][0], '')) if error['loc'] else None
                ))
    ...
    def _convertir_fila_a_producto(self, fila: Dict[str, str]) -> Dict[str, Any]:
        ...
        condiciones = {}
        for campo in ['ca_temp', 'ca_humedad', 'ca_luz', 'ca_ventilacion', 'ca_seguridad', 'ca_envase']:
        ...
        if fila.get('org_fecha_vencimiento') and fila['org_fecha_vencimiento'].strip():
            organizacion['org_fecha_vencimiento'] = fila['org_fecha_vencimiento'].strip()
        
        return {
            'sku': fila.get('sku', '').strip(),
            'nombre_producto': fila.get('nombre_producto', '').strip(),
            'proveedor_id': int(fila.get('proveedor_id', 0)) if fila.get('proveedor_id') else 0,
            ...
            'valor_unitario_usd': float(fila.get('valor_unitario_usd', 0)) if fila.get('valor_unitario_usd') else 0.0,
            'certificaciones_sanitarias': fila.get('certificaciones_sanitarias', '').strip() or None
        }
    ...
    def _convertir_a_producto_create(self, producto_csv: ProductoCSV) -> Dict[str, Any]:
        ...
        return {
            ...
            'tipo_medicamento': producto_csv.organizacion.org_tipo_medicamento if producto_csv.organizacion else None,
            'fecha_vencimiento': producto_csv.organizacion.org_fecha_vencimiento if producto_csv.organizacion else None,
