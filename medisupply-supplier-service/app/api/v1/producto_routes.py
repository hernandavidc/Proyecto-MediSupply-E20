...
async def create_producto(payload: ProductoCreate, db: Session = Depends(get_db)):
    ...
    try:
        producto = service.crear_producto(
            payload.model_dump(), usuario_id=None, origen="MANUAL"
        )
        return producto
...
@router.post(
    "/carga-masiva",
    response_model=ResultadoCargaMasiva,
    status_code=status.HTTP_201_CREATED,
)
async def carga_masiva_productos(
    archivo: UploadFile = File(..., description="Archivo CSV con productos"),
    rechazar_lote_ante_errores: bool = Form(
        True, description="Rechazar todo el lote si hay errores"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error interno al procesar el archivo: {str(e)}"
        )
