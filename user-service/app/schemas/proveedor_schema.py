...
from app.core.enums import (
    PaisOperacion, CertificacionSanitaria, CategoriaSuministro,
    CapacidadCadenaFrio, EstadoProveedor
)


class ProveedorBase(BaseModel):
    """Schema base para Proveedor"""
    razon_social: str = Field(..., min_length=1, max_length=255, description="Razón social del proveedor")
    paises_operacion: List[PaisOperacion] = Field(..., min_items=1, description="Países donde opera (mínimo 1)")
    certificaciones_sanitarias: List[CertificacionSanitaria] = Field(..., min_items=1, description="Certificaciones sanitarias (mínimo 1)")
    categorias_suministradas: List[CategoriaSuministro] = Field(..., min_items=1, description="Categorías de suministros")
    capacidad_cadena_frio: Optional[List[CapacidadCadenaFrio]] = Field(default=None, description="Capacidades de cadena de frío")
    ...
    def validar_razon_social(cls, v):
        ...
        if not v or v.strip() == "":
            raise ValueError("La razón social no puede estar vacía o contener solo espacios")
        return v.strip()
    ...
    def validar_paises_operacion(cls, v):
        ...
        for pais in unique_list:
            if pais not in valid_countries:
                raise ValueError(f"País no válido: {pais}. Países permitidos: {', '.join(valid_countries)}")
    ...
    def validar_certificaciones(cls, v):
        ...
        for cert in unique_certs:
            if cert not in valid_certs:
                raise ValueError(f"Certificación no válida: {cert}. Certificaciones permitidas: {', '.join(valid_certs)}")
    ...
    def validar_categorias(cls, v):
        ...
        for cat in unique_cats:
            if cat not in valid_cats:
                raise ValueError(f"Categoría no válida: {cat}. Categorías permitidas: {', '.join(valid_cats)}")
    ...
    def validar_cadena_frio(cls, v):
        ...
        for cap in unique_caps:
            if cap not in valid_caps:
                raise ValueError(f"Capacidad no válida: {cap}. Capacidades permitidas: {', '.join(valid_caps)}")
...
class ProveedorUpdate(BaseModel):
    ...
    certificaciones_sanitarias: Optional[List[CertificacionSanitaria]] = Field(None, min_items=1)
    categorias_suministradas: Optional[List[CategoriaSuministro]] = Field(None, min_items=1)
